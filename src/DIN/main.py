...
def process_monitor(datapath):
    ...
    date_range = pd.to_datetime(['2010/07/01', '2016/06/30'])
...
obsnme = process_monitor(datapath)

# write the corresponding out instruction file for PEST
write_instruction('output.ins', obsnme)

# write observation ensemble for ies
ies_obs_name = 'observation_ensemble_annual.csv'
observation_ensemble(datapath, ies_obs_name)